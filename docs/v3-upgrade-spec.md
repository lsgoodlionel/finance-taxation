...
| V3 分支 | 当前状态 | 已完成内容 | 下一步 |
|---|---|---|---|
| `codex/v3-design-system-foundation` | 已完成第一批 | 设计 token、全局样式、共享 hooks、PageHeader / EntityDrawer / EmptyState / StepWizard / ResultBanner / DataTableShell 已落地 | 作为后续 V3 页面重构的公共底座复用 |
| `codex/v3-assistant-events-flow` | 已完成第一批 | `AssistantPage / EventsPage` 壳组件化，入口/分析/事项/下游对象主流程表达已落地，部分状态已下沉到子组件 | 后续可继续细化子组件内部状态和 URL 状态收敛 |
| `codex/v3-contracts-page-rework` | 进行中，已完成结构拆分主任务 | `ContractsShell / Header / FiltersBar / ListPanel / Workbench` 已拆分；工作流摘要、元信息、时间轴、对象概览、关联事项、新建表单、列表表格均已下沉；顺手修正 `TasksPage` 旧状态枚举以恢复全量前端 typecheck | 只做 contracts 页视觉一致性和细节收口，不再扩展业务逻辑 |
| `codex/v3-payroll-page-rework` | 进行中，已完成结构拆分第一轮 | shell/header/tab bar 已完成，`employees / payroll / policy` 三段容器已拆出，工资工作流摘要已前置 | 继续拆员工表单、工资明细表与参数表单，保持现有计算/税务/风险联动逻辑不变 |
| `codex/v3-export-center-rework` | 进行中，已完成第一批骨架 | 已建立计划，导出 shell/header/scene selector、历史面板、归档索引面板已拆出 | 继续拆分 8 个导出场景面板，并完成 summary-first 收口 |
| `codex/v3-tax-center-rework` | 已完成，待合回 | TaxPage 已拆为 shell/header/summary/profile/items/batches/materials 多个面板，批次与资料视图收敛到 URL 查询参数 | 合回后进入 Ledger / Reports / Risk |
| `codex/v3-layout-visibility-polish` | 进行中 | 已修复侧栏 footer 遮挡；`Assistant / Contracts / Payroll / Export` 的 hero/section/summary-first 可见性收口已完成 | 同样方法继续补 `Tax / Ledger / Reports / Risk` |
...
6. 当前“结构升级已做，但用户感知不强”的问题仍然存在，剩余结果页必须显式采用 `summary-first`、`hero shell`、`section shell` 和更强的空态/时间线表达。
...
本轮不再只把参考仓库当“灵感来源”，而是明确提炼可直接落到当前代码中的页面模式。

| 模式 | 参考实现 | 对应 V3 页面 |
|---|---|---|
| `PageHeader + 渐变分隔` | `reference/dubbl/components/dashboard/page-header.tsx` | 所有结果页顶区：`Tax / Ledger / Reports / Risk / Export` |
| `Section 左右说明分栏` | `reference/dubbl/components/dashboard/section.tsx` | `Tax / Ledger / Reports / Settings / Knowledge` 的说明型区块 |
| `TabLayout` 顶部轻量导航 | `reference/dubbl/components/dashboard/tab-layout.tsx` | `Ledger / Reports / Tax` 多视图或子场景切换 |
| `StatCard` KPI 卡 | `reference/dubbl/components/dashboard/stat-card.tsx` | `Reports / Risk / Dashboard / Tax` 摘要区 |
| `EmptyState` 大留白空态 | `reference/dubbl/components/dashboard/empty-state.tsx` | `Contracts / Payroll / Export / Risk / Audit` 空态 |
| `AuditTimeline` 垂直时间线 | `reference/dubbl/components/dashboard/audit-timeline.tsx` | `Risk / Audit / Contract timeline / Export audit` |
| `GeneralLedger shell + filter/header/body` | `reference/bigcapital/packages/webapp/src/containers/FinancialStatements/GeneralLedger/GeneralLedger.tsx` | `LedgerPage` 的 `summary-first + filter + statement body` 结构 |
| `Dashboard Stats + FiltersWidget` | `reference/TaxHacker/components/dashboard/stats-widget.tsx`、`filters-widget.tsx` | `Reports / Dashboard / Tax` 顶部摘要卡 + 期间过滤器 |

### 4.2 参考模式映射规则

- `bigcapital` 用于结果页和会计工作台的“壳层 + filter bar + statement body + drawer”组织方式。
- `dubbl` 用于中后台页面的“section / tab / stat-card / empty-state / timeline”视觉层级。
- `TaxHacker` 用于 `AI / OCR / filter widget / dashboard summary` 的场景化入口表达。
- `books` 用于总账、报表、试算平衡、税务底稿的数据展示逻辑和报表密度控制。

后续页重构优先采用“参考模式直接映射”，而不是继续在现有内联样式上局部试错。
...
当前说明：
- 第一轮结构重构已在 `codex/v3-tax-center-rework` 完成。
- 后续只继续做 `summary-first` 可见性收口、批次状态机、打印前校验与资料区响应式，不回退当前面板拆分结构。
...
当前说明：
- 这是当前 V3 下一条高优先级未完成主线。
- 现有 V2 功能已经有：总账分录、余额、过账批次、期间锁账。
- V3 不补底层账务能力，重点是把 `5 个 tab` 重构成清晰的结果页工作台。
...
**参考代码：**
```bash
reference/books/reports/GeneralLedger/GeneralLedger.ts
reference/books/reports/TrialBalance/TrialBalance.ts
reference/bigcapital/packages/webapp/src/containers/FinancialStatements/GeneralLedger/
reference/dubbl/components/dashboard/section.tsx
reference/dubbl/components/dashboard/page-header.tsx
```
...
当前说明：
- drilldown、范围过滤和审计上下文恢复已经在 V2 分支完成。
- V3 不再重做状态模型，只重构展示方式：风险摘要、闭环链路、复盘对象链、审计时间线。
...
**参考代码：**
```bash
reference/dubbl/components/dashboard/audit-timeline.tsx
reference/dubbl/components/dashboard/stat-card.tsx
reference/bigcapital/packages/webapp/src/containers/FinancialStatements/AuditLog/
```
...
当前说明：
- 当前未开始 V3 重构，是 `Ledger` 后的下一条结果页主线。
- 数据能力已具备：三大报表、快照、差异分析、老板摘要、打印。
- V3 重点是 `summary-first + chart view + period compare`，不重写报表计算逻辑。
...
reference/bigcapital/packages/webapp/src/containers/Dashboard/
reference/dubbl/components/dashboard/stat-card.tsx
reference/dubbl/components/ui/chart-line.tsx
reference/TaxHacker/components/dashboard/stats-widget.tsx
```
...
> 截止 `2026-05-27`，`Phase 0` 已完成，`Assistant / Events / Contracts` 已进入 V3 第一阶段实施；后续按 `Payroll -> Export` 顺序推进。
> 截止 `2026-05-28`，`Phase 0 ~ Phase 2` 的主干已基本完成，当前重点转为：`Tax 合回 + Ledger / Reports / Risk + 可见性与响应式收口`。
...
| **Phase 4** | 第7周 | Layout visibility / Responsive / Accessibility / Rnd / Dashboard / Knowledge / Settings | 剩余页体验收口 |
...
#### B. 待开始的高优先级 V3 分支
1. `codex/v3-ledger-page-rework`
   - `summary-first`
   - 5 个 tab 的层级统一
   - 期间/过滤/锁账状态前置
2. `codex/v3-reports-page-rework`
   - KPI + chart view + compare view
   - 报表/快照/差异/老板摘要分区
3. `codex/v3-risk-page-rework`
   - 风险摘要卡
   - 闭环链路可视化
   - 复盘对象链 + 审计时间线
...
- `Accessibility`
  ...
  - 抽屉与步骤区语义
- `Visibility polish`
  - 将当前已经用于 `Assistant / Contracts / Payroll / Export` 的 hero/section/summary-first 规范继续覆盖 `Tax / Ledger / Reports / Risk`
...
新窗口不要直接从旧 `p0 / p1 / p2` 分支继续补历史收口，默认遵循：
