...
export interface PayrollTransferRuntimeBatch {
  id: string;
  status: string;
...
export function deriveTaskRuntimeSummary(tasks: Task[], roleIds: string[]): WorkflowRuntimeSummary {
  ...
  return {
    ...
    actions: blockedTask
      ? [
          {
            key: "retry-blocked-task",
            label: "重开阻塞任务",
            tone: "danger",
            params: { taskId: blockedTask.id }
          }
        ]
      : []
  };
...
export function deriveTaxRuntimeSummary(
  ...
  selectedBatch: TaxFilingBatch | null,
  profiles: TaxpayerProfile[],
  roleIds: string[]
): WorkflowRuntimeSummary {
  const scopedItems = selectedBatch
    ? items.filter((item) => selectedBatch.itemIds.includes(item.id))
    : items;
  const activeProfiles = profiles.filter((item) => item.status === "active").length;
  const selectedBatchStatus = selectedBatch?.status ?? null;
  const scopedReadyItems = scopedItems.filter((item) => item.status === "ready").length;
  const scopedReviewItems = scopedItems.filter((item) => item.status === "review_required").length;
  const readyItems =
    selectedBatchStatus === "ready" || selectedBatchStatus === "submitted" || selectedBatchStatus === "archived"
      ? Math.max(scopedItems.length, scopedReadyItems)
      : scopedReadyItems;
  const reviewItems = selectedBatchStatus === "review_required" ? Math.max(scopedItems.length, scopedReviewItems) : scopedReviewItems;
  ...
  const hasReviewBlock = selectedBatchStatus === "review_required" || (!selectedBatch && reviewItems > 0);

  let executionState: RuntimeExecutionState = "waiting";
  let executionLabel = "等待税务准备";
  let executionMessage = "先确认纳税人口径、税率规则和税务事项，再进入批次复核与申报。";

  if (selectedBatchStatus === "archived") {
    executionState = "succeeded";
    executionLabel = "已留档完成";
    executionMessage = "当前批次已完成复核、申报与归档，后续可直接打印或追溯留痕。";
  } else if (hasReviewBlock) {
    executionState = "failed";
    executionLabel = "待复核修正";
    executionMessage = "存在待复核税务事项，申报批次不应直接进入提交。";
  } else if (selectedBatchStatus === "submitted" || submittedBatches > 0 || readyItems > 0) {
    executionState = "running";
  ...
  if (selectedBatchStatus === "ready" || selectedBatchStatus === "submitted") {
  ...
  const reviewItem = scopedItems.find((item) => item.status === "review_required") ?? null;
        ...
        : hasReviewBlock
          ? {
              ...
              detail: selectedBatch?.id
                ? `建议先对批次 ${selectedBatch.id} 重新复核，再继续提交或归档。`
                : "建议先回到批次复核，再继续提交或归档。"
            }
          : undefined,
    actions:
  ...
  const invalidDetail = detail
    ? (() => {
        ...
        if (Math.abs(debit - credit) > 0.0001) {
          return {
            title: "借贷不平",
            detail: `当前借方 ${debit.toFixed(2)}，贷方 ${credit.toFixed(2)}，需先调整再继续审核过账。`
          };
        }
  ...
  return {
    ...
    actions:
      invalidDetail && detail
        ? [
            {
              key: "retry-voucher-validate",
              label: "重新校验凭证",
              tone: "danger",
              params: { voucherId: detail.id }
            }
          ]
        : []
  };
...
export function derivePayrollTransferRuntimeSummary(
  batches: PayrollTransferRuntimeBatch[],
  selectedBatch: PayrollTransferRuntimeBatch | null,
  roleIds: string[]
): WorkflowRuntimeSummary {
  ...
  if (needsCompensationRepair) {
    ...
    executionMessage = "工资代发已完成，但下游经营事项补偿失败，需要重新执行修复动作。";
  } else if (selectedBatch?.status === "disbursed" || selectedBatch?.status === "confirmed") {
    executionState = "succeeded";
