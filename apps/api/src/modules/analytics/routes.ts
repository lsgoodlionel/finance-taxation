...
/**
 * 默认费用科目前缀：税金及附加 6403、销售费用 6601、管理费用 6602、
 * 财务费用 6603、营业外支出 6711。
 *
 * 旧口径写的是 5601/6601/6602/6603/6711——其中 5601 在科目表里根本不存在，
 * **而当时的 6601 不是销售费用而是「职工薪酬（成本）」**，于是「实际发生额」长期
 * 漏计销售费用、管理费用与财务费用。
 *
 * 绕了一圈之后 `6601` 现在**真的是销售费用**了（D6 迁移 079 废弃职工薪酬、080 把
 * 6201 改成 6601），当年那份口径歪打正着地对上了一半——但它当时确实是错的，
 * 因为那时这个编码指的是另一个科目。
 *
 * 不含主营业务成本 6401 与所得税费用 6801：预算差异比的是期间费用，
 * 与 reports/profit-accounts.ts 的 `expense` 口径（同样剔除 6801）保持一致。
 */
export const EXPENSE_PREFIXES = ["6403", "6601", "6602", "6603", "6711"];
