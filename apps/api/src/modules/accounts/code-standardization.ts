 ...
 *   负债类 2211，生产相关的应进生产成本/制造费用）。D3 当时的判断是「科目设置
 *   本身存疑，在不清楚它承载什么业务的前提下动它风险太大」——**那件事后来查清了**，
 *   见下方「后续（D6）」。
 *
 ...
 *
 * **后续（D6）**：`6601` 已查清——它不承载任何业务（零分录、零余额），结论是废弃。
 * `6201 → 6601` 随之解锁，届时在下面的映射表里加一项即可，
 * **`RETIRED_CODES` 会自动算对，不要手工加 `6601`**：它是新一轮「两头都在」的编码
 *（既是 `6201` 的目标码，又是被废弃科目的旧码），手工塞进去会让护栏把全部销售费用
 * 代码报成待改——D3 在这个判据上已经栽过两次。
 */
