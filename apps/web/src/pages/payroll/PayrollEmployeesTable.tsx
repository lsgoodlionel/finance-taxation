        ...
        <thead>
          <tr style={{ color: "#6c7a89" }}>
            {["姓名", "职位", "入职日期", "基本工资", "状态", "操作"].map((header) => (
              <th key={header} style={{ ...cellStyle(), fontWeight: 500 }}>{header}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {employees.length === 0 ? (
            ...
            employees.map((employee) => {
              ...
              return (
                <tr key={employee.id}>
                  <td style={{ ...cellStyle(), background: highlighted ? "rgba(79,142,247,0.08)" : undefined }}>
                    <div>{employee.name}</div>
                    <div style={{ color: "#8a9bb0", fontSize: "11px" }}>{employee.idCard}</div>
                  </td>
                  <td style={{ ...cellStyle(), background: highlighted ? "rgba(79,142,247,0.08)" : undefined }}>{employee.position || "—"}</td>
                  <td style={{ ...cellStyle(), background: highlighted ? "rgba(79,142,247,0.08)" : undefined }}>{employee.hireDate ?? "—"}</td>
                  <td style={{ ...cellStyle(), background: highlighted ? "rgba(79,142,247,0.08)" : undefined }}>¥ {formatAmount(employee.baseSalary)}</td>
                  <td style={{ ...cellStyle(), background: highlighted ? "rgba(79,142,247,0.08)" : undefined }}>
