...
type ReportsSidebarProps = ReportsPeriodState & {
  /** V7 K3 双轨：guided 摘要优先 + 专业能力折叠；缺省 pro 完全不变。 */
  mode?: "guided" | "pro";
...
/** guided 模式下折叠进「查看专业报表」的视图（老板摘要之外的全部）。 */
const professionalViews: ReportsWorkbenchView[] = views.filter((view) => view !== "chairman");
...
export function ReportsSidebar(props: ReportsSidebarProps) {
  ...
  function renderViewButton(view: ReportsWorkbenchView) {
    ...
    return (
      <button
        ...
        style={{
          ...
          minHeight: isGuided ? 44 : undefined
        }}
      >
        {isGuided && view === "chairman" ? "老板摘要（白话讲结论）" : getWorkbenchViewLabel(view)}
      </button>
    );
  }
  ...
  const periodSection = (
    <section style={cardStyle()}>
      <div style={{ display: "grid", gap: "4px" }}>
        ...
        <strong style={{ fontSize: "16px", color: "#1e2a37" }}>
          {isGuided ? "先选看哪个月，再看结论" : "先固定报表口径，再决定查看和输出"}
        </strong>
      </div>
      <div
        style={{
    ...
    <section style={cardStyle()}>
      <div style={{ display: "grid", gap: "4px" }}>
        ...
        <div style={{ fontSize: "13px", color: "#4d5d6c" }}>
          {isGuided
            ? "默认给您白话结论；想看专业报表随时展开。"
            : "右侧只显示当前关注的结果，避免三表与分析结果混在同一长页里。"}
        </div>
      </div>
      {isGuided ? (
        <div style={{ display: "grid", gap: "8px" }}>
          {renderViewButton("chairman")}
          <details>
            <summary style={detailsSummaryStyle()}>查看专业报表</summary>
            <div style={{ display: "grid", gap: "8px", marginTop: "8px" }}>
              {professionalViews.map(renderViewButton)}
            </div>
          </details>
        </div>
      ) : (
        <div style={{ display: "grid", gap: "8px" }}>{views.map(renderViewButton)}</div>
      )}
    </section>
  );
  ...
  return (
    <aside style={{ display: "grid", gap: "20px", position: "sticky", top: "16px" }}>
      ...
      {viewSection}
      {isGuided ? (
        <details style={{ display: "grid", gap: "20px" }}>
          <summary style={detailsSummaryStyle()}>财务专业工具（快照 / 对比 / 打印输出）</summary>
