...
系统共 **24+ 个业务页面**，覆盖"经营事项 → 账务 → 税务 → 报表 → 风险 → 归档"完整闭环，并叠加合同、工资、AI Agent、知识库、审计、计费、往来画像、资料包等高阶功能：

| 页面 | 路由 | 核心能力 |
|------|------|----------|
| 董事长驾驶舱 | `/dashboard/chairman` | 银行余额 / 利润 / 税负 / 风险事项 / AI 摘要 |
| 我的工作台 | `/my-day` | 待办收件箱 / 当日任务 / 全局搜索 ⌘K |
| 经营事项总线 | `/events` | 创建 / 编辑 / AI 拆任务 / 风险检查 / 跨页导航 / 财税链路条 |
| 任务中心 | `/tasks` | 任务树 / 状态流 / 逾期高亮 / 催办徽章 |
| 单据中心 | `/documents` | 附件上传（multipart）/ 归档 / 正式财务单据格式展示 / 资料完整性 Agent |
| 凭证中心 | `/vouchers` | 模板生成 / 校验 / 审核 / 过账 / 正式记账凭证表格 / 发票→凭证 |
| 总账中心 | `/ledger` | 分录列表 / 科目余额 / 过账批次 / 日记账 Tab / 期间锁账 |
| 财务报表 | `/reports` | 三表 + 快照 + 差异分析 + 老板摘要 + 打印版 + 报表→申报直达 |
| 税务中心 | `/tax` | 增值税底稿 / 企所税 / 个税 / 印花税 / 申报批次 / 复核留档 / 到期提醒 |
| 研发辅助账 | `/rnd` | 研发项目 / 费用归集 / 加计扣除资料包 / 月度趋势 |
| 风险勾稽 | `/risk` | 规则引擎 / 风险发现 / 异常关闭复盘 / AI 审计勾稽 Agent |
| 合同管理 | `/contracts` | 合同主数据 / 看板拖拽 / 状态流 / 关联事项 / 履约时间轴 |
| 工资管理 | `/payroll` | 员工档案 / IIT 七级计算 / 社保公积金 / 工资向导 / 代发 |
| 工资代发 | `/payroll-transfer` | 银行代发批次 / 账号批量管理 |
| AI 财税秘书 | `/assistant` | 流式对话 / 财税问答 / 建议事项一键创建 / PDF 直传识别 / 标准业务流程图 |
| PDF 导出 | `/pdf-export` | 工资汇总 / 工资条 / 凭证 / 报表快照 / 场景化导出 |
| 月度结账向导 | `/month-end-close` | 结账检查清单 / 步骤引导 / 全局期间控制 |
| 审计日志 | `/audit` | 操作人 / 时间 / 模块 / 变更详情全量追踪 / AI 审计 Agent |
| 老板专线 | `/boss-qa` | 实时财务快照注入 + SSE 流式问答 + 资金前瞻 |
| 企业知识库 | `/knowledge` | 制度条款 / 口径规则 / AI Prompt 注入 / PDF & Word 批量上传 |
| 往来单位 | `/counterparties` | 往来单位主数据 / 应收应付画像 |
| 财税资料包 | `/archive-package` | 期间归档 / 凭证+报表+申报+审计打包导出 |
| 订阅计费 | `/billing` | 套餐 / 订阅 / 配额 / 支付 |
| 反馈与建议 | `/feedback` | 用户反馈 / 升级需求 / 产品建议 |
